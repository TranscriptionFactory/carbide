...
export function create_native_agent_tauri_adapter(): AgentPort {
  return {
    stream_turn(input: AgentStreamRequest): AsyncIterable<AgentEvent> {
      ...
      void (async () => {
        const unlisten = await listen<AgentEvent>(
          `native-agent-stream-event:${request_id}`,
          (event) => {
        ...
        const on_abort = () => {
          void tauri_invoke("native_agent_stream_abort", {
        ...
        try {
          await tauri_invoke("native_agent_stream_start", {
            requestId: request_id,
