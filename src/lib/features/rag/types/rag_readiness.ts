export type RagReadiness =
  ...
  // a vector (unembeddable content, a failed encode), so `skipped` here is the
  // notes it never selects and `total - embedded` is the deficit. Terminal until
  // some later attempt changes the numbers.
