...
<div class="h-full flex flex-col bg-background">
  ...
  >
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-0.5 shrink-0">
        <button
          class="p-1 hover:bg-accent rounded-md transition-colors {views_open
            ? 'text-blue-500'
            : ''}"
        ...
        <button
          class="p-1 hover:bg-accent rounded-md transition-colors {filters_open ||
          has_filters
            ? 'text-blue-500'
            : ''}"
  ...
  {#if views_open}
    ...
    >
      ...
      {#if bases_store.saved_views.length > 0}
        <div class="space-y-1">
          <span class="text-[10px] text-muted-foreground">Saved views</span>
          {#each bases_store.saved_views as view}
            <div class="flex items-center gap-2 text-xs">
              <button
                type="button"
                class="flex-1 text-left px-2 py-1.5 bg-muted rounded-md hover:bg-accent truncate {bases_store.active_view_name ===
                view.name
              ...
              <button
                class="p-1 hover:bg-red-100 dark:hover:bg-red-900/30 rounded text-muted-foreground hover:text-red-500"
      ...
      {:else}
        <div class="text-xs text-muted-foreground">No saved views yet.</div>
      {/if}
    </div>
  {/if}
    ...
    >
      {#if bases_store.query.filters.length > 0}
        <div class="space-y-1.5">
          {#each bases_store.query.filters as filter, i}
            <div class="flex items-center gap-2 text-xs">
              <span
                class="px-2 py-1 bg-muted rounded font-medium truncate max-w-[120px]"
      ...
      <div class="flex items-end gap-2">
        ...
        <div class="w-20">
          <label
            for="filter-operator"
            class="block text-[10px] text-muted-foreground mb-0.5">Op</label
          >
          <select
            id="filter-operator"
    ...
    {:else}
      <div class="p-4 space-y-4">
        {#each bases_store.result_set as row}
          <button
            type="button"
            class="w-full text-left p-3 border border-border rounded-lg hover:border-border/60 transition-all cursor-pointer bg-card"
  ...
  {#if bases_store.total_count > bases_store.query.limit}
    <div
      class="flex items-center justify-between px-4 py-2 border-t border-border text-xs text-muted-foreground"
    >
      <span
      ...
      <div class="flex items-center gap-2">
        <button
          class="px-2 py-1 bg-muted rounded hover:bg-accent disabled:opacity-50"
