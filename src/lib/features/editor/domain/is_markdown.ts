...
function sample_for_heuristic(text: string): string {
...
export function is_markdown(text: string): boolean {
  if (!text) return false;
  const sample = sample_for_heuristic(text);
  let signals = 0;
  ...
  if (
    STRONG_STAR_RE.test(sample) ||
    STRONG_UNDER_RE.test(sample) ||
    STRIKE_RE.test(sample)
  )
    signals++;
  ...
  const line_count = sample.split("\n").length;
  const threshold = Math.min(3, Math.floor(line_count / 5));
