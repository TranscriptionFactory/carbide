...
const RECENTS_CUSTOM_RANGE = /^\d{4}-\d{2}-\d{2}\.\.\d{4}-\d{2}-\d{2}$/;
...
export function coerce_recents_period(value: unknown): RecentsPeriod {
  ...
  if (
    typeof value === "string" &&
    value.startsWith(RECENTS_CUSTOM_PREFIX) &&
    RECENTS_CUSTOM_RANGE.test(value.slice(RECENTS_CUSTOM_PREFIX.length))
  ) {
