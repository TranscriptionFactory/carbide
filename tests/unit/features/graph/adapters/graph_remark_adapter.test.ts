...
function make_notes_port(docs: NoteDoc[]): NotesPort {
  const doc_map = new Map(docs.map((d) => [d.meta.id, d]));
  return {
    list_notes: vi.fn().mockResolvedValue(docs.map((d) => d.meta)),
    read_note: vi
      .fn()
      .mockImplementation((_vault_id: VaultId, note_id: NoteId) => {
        const doc = doc_map.get(note_id);
        return doc
          ? Promise.resolve(doc)
...
describe("create_graph_remark_adapter", () => {
  it("builds vault graph from wikilinks", async () => {
    const port = make_notes_port([
      make_doc("a.md", "Link to [[b]]"),
      make_doc("b.md", "Link to [[a]]"),
      make_doc("c.md", "No links"),
    ]);

    const adapter = create_graph_remark_adapter(port);
  ...
  it("identifies orphan links", async () => {
    const port = make_notes_port([
      make_doc("a.md", "Links to [[nonexistent]]"),
    ]);
