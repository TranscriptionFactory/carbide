...
describe("citations_from_tools", () => {
  ...
  it("normalizes absolute harness paths to vault-relative", () => {
    const citations = citations_from_tools(
      [
        {
          name: "Read",
          input_summary: '{"file_path":"/vault/root/notes/a.md"}',
        },
      ],
      vault,
    );
  ...
  it("prefers structural paths over a truncated summary", () => {
    const citations = citations_from_tools(
      [
        {
          name: "Read",
          input_summary: '{"content":"…","file_path":"/vault/ro',
          paths: ["/vault/root/notes/b.md"],
        },
      ],
      vault,
    );
    expect(citations).toEqual([
      { index: 1, note_path: "notes/b.md", title: "b" },
    ]);
  });

  it("deduplicates absolute and relative forms of the same note", () => {
    const citations = citations_from_tools(
      [
        { name: "mcp__carbide__read_note", input_summary: '{"path":"a.md"}' },
        { name: "Read", input_summary: '{"file_path":"/vault/root/a.md"}' },
      ],
      vault,
    );
  ...
  it("falls back to no citation when a JSON summary is truncated and no structural paths exist", () => {
    const citations = citations_from_tools(
