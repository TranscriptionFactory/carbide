...
describe("extract_mermaid_sources", () => {
  ...
  it("returns all sources when multiple blocks are present", () => {
    const html = '<pre class="mermaid">a</pre>x<pre class="mermaid">b</pre>';
...
describe("inject_mermaid_svgs", () => {
  it("replaces matching blocks with <figure> wrappers", () => {
    ...
    expect(out).toBe(
      "<p>hi</p><figure class=\"mermaid-figure\"><svg id='a'/></figure>",
    );
  });

  it("leaves blocks without a matching svg untouched", () => {
    const html =
      '<pre class="mermaid">graph A</pre><pre class="mermaid">graph B</pre>';
    const out = inject_mermaid_svgs(html, new Map([["graph A", "<svg/>"]]));
