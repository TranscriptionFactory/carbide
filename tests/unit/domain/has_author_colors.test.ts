...
describe("has_author_colors", () => {
  it("detects color and background declarations in style attributes", () => {
    ...
    expect(has_author_colors(`<div style="background:#fff">x</div>`)).toBe(
      true,
    );
    expect(
      has_author_colors(
        `<div style='padding:4px;background-color:#fff'>x</div>`,
      ),
    ).toBe(true);
    expect(
  ...
  it("detects legacy color attributes", () => {
    ...
    expect(
      has_author_colors(`<table bordercolor="#eee"><tr></tr></table>`),
    ).toBe(true);
    expect(has_author_colors(`<font color="red">x</font>`)).toBe(true);
  });

  it("ignores unstyled content and non-color declarations", () => {
    expect(has_author_colors(`<p>hello <strong>world</strong></p>`)).toBe(
      false,
    );
