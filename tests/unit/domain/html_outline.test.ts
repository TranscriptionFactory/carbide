...
describe("extract_html_headings", () => {
  it("extracts nested headings with stable occurrence keys", () => {
    expect(
      extract_html_headings("<h1 id='top'>Top</h1><section><h2>Child</h2></section>"),
    ).toEqual([
      { id: "top", level: 1, text: "Top", pos: 0 },
      { id: "carbide-heading-1", level: 2, text: "Child", pos: 1 },
    ]);
  });

  it("normalizes malformed heading contents to text", () => {
    expect(extract_html_headings("<h2>Hello <em>world</em></h2>")[0]?.text).toBe(
      "Hello world",
    );
  });
});
