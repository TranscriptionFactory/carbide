...
2. **B2**: Ship default saved views. — *done* — `bases_seed_default_views` Tauri command at `src-tauri/src/features/bases/service.rs` writes the six seeds to `.carbide/bases/{slug}.json` on first vault open; sentinel `.carbide/bases/.seeded` guards re-seeding. `bases_refresh` reactor at `src/lib/reactors/bases_refresh.reactor.svelte.ts` calls it after `refresh_properties`. Seeds requiring a frontmatter property (`created`, `status`) are skipped when that property isn't in the vault's `note_properties`. **Resolved (2026-05-30):** `Modified This Week` / `Smart Archive` no longer depend on frontmatter — `query_bases` now substitutes `now()`/`now()-Nd` relative-date expressions into epoch-ms and maps the friendly date properties `modified`→`mtime_ms`, `created`→`ctime_ms`, `accessed`→`mtime_ms` (accessed is a modified-time proxy; no atime is tracked). Both seeds were ungated (`required_property: None`) so they now seed and return rows. Helpers `resolve_bases_property` / `resolve_bases_now_value` + tests in `src-tauri/src/features/search/db.rs`. **Still gated:** `Orphan Notes` (`backlink_count`) — backlinks aren't a computed column yet, so it remains skipped until that lands (out of scope for the now()/mtime work). Rust tests in `src-tauri/src/features/bases/service.rs` under `mod tests`.
...
9. Path-aware Cmd+P (#26). — *done* — `open_omnibar` in `src/lib/features/search/application/omnibar_actions.ts` now checks `document.activeElement.closest('[data-vim-nav-region="file_tree"]')` when the omnibar is opened with no existing query. When focus is inside the file tree *and* a folder is selected, the omnibar pre-fills with `<selected_folder>/` and immediately runs the path-prefixed search. Falls through to existing behavior otherwise. Tests in `tests/unit/actions/register_omnibar_actions.test.ts` cover both the focused and unfocused branches by stubbing `globalThis.document`. Chose DOM-based focus detection over wiring `vim_nav` into `ActionRegistrationInput` to keep the type signature stable across the dozen+ action test fixtures.
...
3. Color + icon polish driven by frontmatter (#14 + #22). — *done* — `NoteMeta` gained `color`/`icon` optional fields on both runtimes (Rust `src-tauri/src/features/notes/service.rs`, TS `src/lib/shared/types/note.ts`, generated bindings). Rust frontmatter scanner generalized: `extract_frontmatter_str_field(text, key)` (existing `extract_frontmatter_description` now delegates) plus `extract_color_icon(path)` that reads the file head once and pulls both values. `build_note_meta` and `write_and_index_note` populate the fields, and `WriteAndIndexResult` carries them so the save flow keeps the store fresh (new `NotesStore.update_note_after_save(id, blurb, color, icon)`; both save sites in `note_service.ts` switched over). Render layer: a tiny domain helper at `src/lib/features/folder/domain/note_visuals.ts` sanitizes the values (hex/named-color allowlist for color; trimmed glyph capped at 8 codepoints for icon). `file_tree_row.svelte` swaps the default `<FileText />` for a `<span class="TreeRow__type-icon--emoji">` when an icon is set, and renders a 2px left accent stripe (`TreeRow__color-stripe`) driven by a `--note-color` CSS variable — uses a real DOM element to avoid clashing with the existing `::before` indent guides and the `::after` active-state bars across the 5 tree styles. `DrillDownFileTree` mirrors the same treatment with locally-scoped `DrillRow*` styles. Tests in `tests/unit/domain/note_visuals.test.ts`. **Scope limit (v1):** notes only — folder rows don't yet inherit icon/color from their `folder/folder.md` landing page (follow-up: thread a `folder_note_meta` lookup through `flatten_filetree` or resolve at the call site against `notes_store`).
