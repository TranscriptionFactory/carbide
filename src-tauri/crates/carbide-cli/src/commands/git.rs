...
pub async fn run(
    ...
    match action {
        ...
        GitAction::Log { limit, file } => {
            log(client, vault_id, *limit, file.as_deref(), json).await
        }
...
async fn push(client: &CarbideClient, vault_id: &str, json: bool) -> Result<(), String> {
    ...
    if json {
        ...
        println!(
            "{}",
            resp["message"].as_str().unwrap_or("Pushed successfully")
        );
    } else {
...
async fn pull(
    ...
    if json {
        ...
        println!(
            "{}",
            resp["message"].as_str().unwrap_or("Pulled successfully")
        );
    } else {
