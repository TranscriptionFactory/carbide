  ...
  unsafe {
    ...
    if let Some(function) = function {
      ...
      let Some(url) = request.URL() else {
        #[cfg(feature = "tracing")]
        tracing::warn!("wry: custom scheme request has nil URL, ignoring");
        eprintln!("[wry patch] custom scheme request has nil URL — dropping request");
        return;
      };

      let Some(abs) = url.absoluteString() else {
        #[cfg(feature = "tracing")]
        tracing::warn!("wry: custom scheme URL has nil absoluteString, ignoring");
        eprintln!("[wry patch] custom scheme URL has nil absoluteString — dropping request");
        return;
      };
      ...
      let method = match request.HTTPMethod() {
        ...
        None => {
          #[cfg(feature = "tracing")]
          tracing::warn!("wry: custom scheme request has nil HTTPMethod for {}, defaulting to GET", uri);
          eprintln!("[wry patch] custom scheme request has nil HTTPMethod for {} — defaulting to GET", uri);
          "GET".to_string()
        }
      };
