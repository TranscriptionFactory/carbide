...
pub async fn spawn_native_run(
    app: AppHandle,
    request_id: String,
    event_name: String,
    spec: super::agent_stream::AgentRunSpec,
) {
    let AiTransport::Api { .. } = &spec.provider_config.transport else {
        let _ = app.emit(
            &event_name,
            AgentEvent::Error {
                message: format!("{} does not support native agent mode", spec.provider_config.name),
            },
        );
        return;
    };

    let _ = spec.resume_session_id;

    let (abort_tx, abort_rx) = oneshot::channel::<()>();
    app.state::<super::agent_stream::AgentRunState>()
        .insert_handle(request_id.clone(), abort_tx)
        .await;
    ...
    let emit = move |event: AgentEvent| {
        let _ = emit_app.emit(&emit_event_name, event);
    };
    ...
    tokio::spawn(async move {
        ...
        .await;
        app_clone
            .state::<super::agent_stream::AgentRunState>()
